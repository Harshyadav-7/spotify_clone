...
async function getSongs(folder) {
    ...
    const response = await fetch(`http://127.0.0.1:65081/${folder}/`);
...
async function displayAlbums() {
    const response = await fetch(`http://127.0.0.1:65081/songs/`);
    const html = await response.text();
    ...
    for (let e of anchors) {
        if (e.href.includes("/songs/")) {
            ...
            try {
                const res = await fetch(`http://127.0.0.1:65081/songs/${folder}/info.json`);
